...
TEST_CASE("spdlog_test") {
	init_log(LogSettings{.name = "log_test"});

	auto logger = spdlog::default_logger();

	logger->info("Welcome to spdlog version {}.{}.{}  !",
	             SPDLOG_VER_MAJOR,
	             SPDLOG_VER_MINOR,
	             SPDLOG_VER_PATCH);
...
TEST_CASE("spdlog_bench") {
	init_log(LogSettings{.name = "log_bench"});
	auto howmany = 200000;
	...
	auto logger1 = spdlog::rotating_logger_mt(
		"rotating_mt", "logs/bench_sync_rotating.log", 1024 * 1024 * 10, 5);
